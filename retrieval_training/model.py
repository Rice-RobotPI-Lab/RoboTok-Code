...
class DTWLayer(nn.Module):
    ...
    def __init__(
        ...
        # Gaussian init scaled to the input standard deviation.
        self.prototypes = nn.Parameter(
            torch.randn(num_nodes, prototype_length, feature_dim) * input_std
        )
        # BatchNorm over the per-prototype similarity activations (no ReLU
        # before BN, unlike Iwana et al.).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Path-length normalisation: divide by max(T, L) so the activation is
        # on the per-step cost scale and independent of clip length.
...
class CrossAttentionProjectionHead(nn.Module):
    ...
    def forward(
        ...
        if self.dtw_nn_layer:
            ...
            # then maps to retrieval_dim. The entire DTW head runs in fp32.
            with torch.amp.autocast("cuda", enabled=False):
        ...
        # For num_queries == 1 the combiner is Identity and this reduces to
        # q.squeeze(0).
