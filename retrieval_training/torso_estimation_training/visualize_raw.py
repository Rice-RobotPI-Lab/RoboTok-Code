"""
Visualize VN torso predictions using only torso_relative_clip_keypoints.pt.

Recomputes camera transforms and wrist frames from raw keypoints — no
dependency on depth_grounded_clip_keypoints_transforms.pt.

Usage:
    python visualize_raw.py --vn-checkpoint ../../models/body_pose_est.pt
"""
...
RAW_PATH = DIR.parents[1] / "eval_data" / "torso_relative_clip_keypoints.pt"
# The retargeting code is an external dependency (see README).
RETARGET_DIR = Path(
    os.environ.get("RETARGET_DIR", DIR.parents[2] / "external_retargeting" / "retarget")
)
SMPLH_MODEL_PATH = DIR / "neutral" / "model.npz"
...
def _invert_se3(T):
    ...
    R_inv = R.T
    t_inv = -R_inv @ t
    return _se3_from_R_t(R_inv, t_inv)
...
def _build_clip_mesh(raw_clip, vn_model, vn_device, render_frame_key=None):
    ...
    for fk in render_keys:
        ...
        # SMPL-H T-pose mesh at predicted torso
        if vn_pred_world is not None:
            R_vn, t_vn = vn_pred_world
            smpl_mesh = _smplh_tpose_mesh_at(R_vn, t_vn)
...
def main():
    ...
    parser.add_argument("--vn-checkpoint", type=str, required=True,
