...
Loads the clip-keypoints cache, applies the configured DTW design transform
and writes a single packed `.pt` artifact. Used at training time to compute
per-batch DTW online without re-doing the keypoints → trajectory transform
on every step.
...
Also emits `clip_index_{design}.json` next to the `.pt` mapping row index
-> (video_number, node_uid). Row order is `bundle.clips` order, and the
artifact's `clip_keys` is what `VideoClipDataset` reads.
...
CLI:
    ...
    python build_trajectories.py ... -n 50           # cap to first 50 videos
"""
...
def build_trajectories(
    ...
    Args:
        data_dir:    directory with `clip_keypoints.pt` /
                     `depth_grounded_clip_keypoints.pt`.
        dtw_design:  key into `DTW_DESIGNS` from dtw_designs.py.
    ...
    t0 = time.time()
    bundle = load_data_bundle_21j(num_videos)
    ...
    # Emit clip_index_{design}.json next to the artifact, mapping
    # clip_idx -> (video_number, node_uid) in `bundle.clips` (= row) order.
...
def main() -> None:
    ...
    p.add_argument("--design", type=str, required=True,
                   help="DTW design key (see dtw_designs.DTW_DESIGNS)")
