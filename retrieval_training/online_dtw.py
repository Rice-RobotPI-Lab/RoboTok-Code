"""Online DTW similarity for training targets and evaluation ground truth.

Computes exact (length-aware) symmetric-2 DTW between hand trajectories on
the GPU. Every pair `(i, j)` is aligned on its true `L_i × L_j` prefix grid,
so the boundary-repeat padding used to batch clips to a common `T_max` never
contributes to the distance. The same cost / kernel / normalisation is used
by the in-batch target (`compute`), the eval-set matrix (`compute_chunked`)
and the neighbor-table builder (`build_dataset/build_dtw_neighbors.py`), so
all three measure the same quantity.

Design semantics (cost / kernel / norm) come from `dtw_designs.py` via
...
# dtw_designs.py lives alongside this module; make it importable regardless of
...
class OnlineDTWComputer:
    ...
    def __init__(self, dtw_design: str):
        from dtw_designs import DTW_DESIGNS, DTW_METHODS, DTW_NORMS
    ...
    def compute(self, trajs: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            [B, B] float32 similarity = negated exact DTW distance, on
            `trajs.device`. Diagonal is forced to exactly 0 (distance to
            self is 0 under symmetric2; forcing it absorbs kernel FP noise).
        """
        ...
        B = trajs.shape[0]
        sim = torch.zeros(B, B, dtype=torch.float32, device=trajs.device)
        # Iterate query-by-query so the cost tensor stays [B, L_i, L_j], not
        # [B, B, T, T].
        for i in range(B):
            sim[i] = self.compute_one_query_against_targets(
                trajs[i], trajs, lengths_f[i], lengths_f
            )
        # Symmetric-2 DTW is mathematically symmetric with zero self-distance.
    ...
    def compute_one_query_against_targets(
        ...
        """Rectangular query→targets exact DTW.

        Buckets targets by their true length `L_j` and dispatches one kernel
        call per bucket on the truncated `L_i × L_j` grid; the distance is
        read from `R[L_i, L_j]`, the corner of the real prefix grid, so
        padded frames are never visited.
        ...
        for L_j_t in torch.unique(target_lengths_f):
            ...
            d = self.compute_block(q_trunc, tgt, q_len_t, tgt_lens_f)
    ...
    def compute_block(
        ...
        """One kernel call on an exact `L_i × L_j` grid (no padding)."""
    ...
    def compute_chunked(
        ...
        Like `compute(...)` but chunks target columns so the intermediate
        cost tensor stays small. Use this for eval-set–scale matrices
        (N ~ 10k). Result is identical to `compute(...)` — the per-chunk
        body is delegated to `compute_one_query_against_targets`.
        """
        if trajs.device.type != "cuda":
