...
class VideoClipDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if self.load_jepa_features:
            ...
            # .to(device) in the train/eval loops.
...
class JepaPooledDataset(Dataset):
    ...
    instead of padded `[T, D]` blocks. Used by `train._build_eval_jepa_sim`
    to compute the eval JEPA similarity matrix.
    """
...
def select_model_input(
    ...
    Shared by train.py and eval.py so the input-selection logic lives in
    exactly one place.
