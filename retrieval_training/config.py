...
class Config:
    ...
    # on a different temporal aspect of the clip; with num_queries == 1 the
    # combiner is the identity.
    num_queries: int = 1
    use_grad_checkpoint: bool = True  # gradient checkpointing in the cross-attention head
    ...
    num_workers: int = 8
    # Keep DataLoader workers alive across epochs (False re-spawns them each epoch).
    ...
    # every clip in the run. Uses the cached global DTW-neighbor table.
    ...
    # Inject precomputed top-K DTW neighbors into each batch as positives.
    # See `positive_mining_sampler.py` and
    # `build_dataset/build_dtw_neighbors.py`. The neighbor cache is rebuilt
    # per run inside `run_dir`.
    use_positive_mining: bool = True           # toggle the sampler
    ...
    # FAISS prefilter settings (soft_contrastive only). When
    # loss_type == "topk_set_rank" the neighbor cache is always exact
    # all-pairs DTW over the full corpus (no FAISS) and these two are ignored.
