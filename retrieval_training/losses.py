...
def _normalize_matrix(mat: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    `_DEGENERATE_STD_WARN_THRESHOLD`.
    """
    ...
    if std < _DEGENERATE_STD_WARN_THRESHOLD:
        print(f"[losses] WARNING: target std={std:.2e} is degenerate "
              f"(< {_DEGENERATE_STD_WARN_THRESHOLD:.0e}).")
    return out
...
class SimilarityAlignmentLoss(nn.Module):
    ...
    `loss_type` is a switch over the alignment families this class implements;
    `"soft_contrastive"` is the only one. The set/rank objective lives in
    `TopKSetRankLoss`.
    """
...
class TopKSetRankLoss(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Cache data is run-specific supervision, not model state, so keep it
        # out of checkpoints.
