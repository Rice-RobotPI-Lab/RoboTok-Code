...
def encode_dataset(
    ...
    `Subset`, so the encoded N always matches the precomputed eval matrices.
    """
    model.eval()
...
def encode_dataset_with_loss(
    ...
    held-out subset instead of a single train batch.
    """
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Exact DTW under autocast-disabled fp32 to match the train regime.
...
def _eval_embedding_health(z: np.ndarray, device: torch.device) -> Dict[str, float]:
    ...
    Mirrors the train-side `_z_collapse_stats` / `_off_diag_stats` so the
    train and eval values are directly comparable.
    """
...
def run_evaluation(
    ...
    directly comparable to the corresponding train-side scalars.
    """
