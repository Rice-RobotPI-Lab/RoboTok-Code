...
def _build_eval_dtw_matrix(
    ...
    """Build the (N_eval, N_eval) exact-DTW similarity matrix once at startup.

    Indexed by eval-position (the order eval_dataloader yields clips with
    shuffle=False). This is the evaluation ground truth for the held-out
    Recall@K / CKNNA metrics.
    """
    base, positions = _resolve_subset(eval_dataset)
    ...
    print(f"Building eval DTW matrix (exact DTW): N_eval={len(positions)} ...")
    t0 = time.perf_counter()
    eval_sims = dtw_computer.compute_chunked(
        eval_trajs, eval_lens, target_chunk=1024
    )
    elapsed = time.perf_counter() - t0
...
def initialize_query_token(
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # fp32 materialisation of the full [B, T, D] tensor.
            ...
            # Sum reduction in fp32 to avoid fp16 overflow.
...
def main():
    ...
    # Train-pool CKNNA cache (train queries x train candidates); path and
    # top_k are needed here because the set/rank path builds it in the same
    # exhaustive pass as the main cache.
    train_neigh_path = run_dir / f"dtw_neighbors_{cfg.dtw_design}_train_pool.pt"
    train_pool_topk = max(
        cfg.cknna_topk, max(cfg.recall_k_values), max(cfg.cknna_k_values)
    )
    # topk_set_rank supervises directly from the cached DTW table, so that
    # table must be the *true* corpus-wide DTW top-K: exact DTW over every
    # pair, no FAISS candidate prefilter. This overrides
    # `positive_mining_candidate_k` / `positive_mining_pooling` unconditionally.
    exhaustive_dtw_cache = set_rank_mode
    if need_dtw_neighbor_cache:
        ...
        reuse_neigh = neigh_path.exists()
        if reuse_neigh and exhaustive_dtw_cache:
            _existing = torch.load(neigh_path, weights_only=False, map_location="cpu")
            if _existing.get("dtw_mode") != "exact_all_pairs":
                print(
                    f"Existing DTW neighbor cache at {neigh_path} is "
                    f"dtw_mode={_existing.get('dtw_mode')!r}; topk_set_rank "
                    "requires exact_all_pairs -> rebuilding"
                )
                reuse_neigh = False
            del _existing
        if not reuse_neigh:
            ...
            if _bd not in _sys.path:
                _sys.path.insert(0, _bd)
            if exhaustive_dtw_cache:
                from build_dtw_neighbors import build_dtw_neighbors_exhaustive_from_payload
                print(
                    f"Building exact all-pairs DTW neighbors for "
                    f"design={cfg.dtw_design} -> {neigh_path} "
                    f"(+ train-pool table -> {train_neigh_path})"
                )
                _traj_payload = torch.load(
                    traj_path, weights_only=False, map_location="cpu"
                )
                # Same split the dataset section computes below: it depends
                # only on (N, holdout_fraction, seed) and the dataset has one
                # row per trajectory, so this is the identical partition.
                _n_traj = int(_traj_payload["trajectories"].shape[0])
                _train_positions, _ = random_holdout_split(
                    _n_traj, cfg.eval_holdout_fraction, cfg.seed,
                )
                build_dtw_neighbors_exhaustive_from_payload(
                    ...
                    train_pool_top_k=train_pool_topk,
                    train_pool_output_path=train_neigh_path,
                )
                del _traj_payload
            else:
    ...
    if need_dtw_neighbor_cache:
        ...
        if exhaustive_dtw_cache:
            if neigh.get("dtw_mode") != "exact_all_pairs":
                raise RuntimeError(
                    f"topk_set_rank requires an exact all-pairs DTW neighbor "
                    f"cache; got dtw_mode={neigh.get('dtw_mode')!r} at {neigh_path}"
                )
        elif neigh.get("pooling") != cfg.positive_mining_pooling:
            raise RuntimeError(
    ...
    if cfg.eval_global_cknna:
        ...
        _neigh_source = (
            ...
            if neigh.get("dtw_mode") == "exact_all_pairs"
            else f"candidate_k={neigh['candidate_k']}, pooling={neigh['pooling']}"
        )
        print(
            f"Global-pool CKNNA enabled: {len(eval_positions)} eval queries | "
            f"{len(dataset)} total candidates | cached DTW top-{neigh['top_k']} "
            f"from {_neigh_source}"
        )

        # -- Train-pool CKNNA: up to `eval_max_clips` random train queries vs
        # the train-side corpus. Mirrors the global block but restricted to
        # train-side data.
        # `train_pool_topk` / `train_neigh_path` are defined alongside the
        # main cache above. In set/rank mode the table was already written by
        # the exhaustive pass; this fallback only fires if it is missing or
        # was built by the FAISS path.
        reuse_train_neigh = train_neigh_path.exists()
        if reuse_train_neigh and exhaustive_dtw_cache:
            _existing = torch.load(
                train_neigh_path, weights_only=False, map_location="cpu",
            )
            if _existing.get("dtw_mode") != "exact_all_pairs":
                print(
                    f"Existing train-pool cache at {train_neigh_path} is "
                    f"dtw_mode={_existing.get('dtw_mode')!r}; topk_set_rank "
                    "requires exact_all_pairs -> rebuilding"
                )
                reuse_train_neigh = False
            del _existing
        if not reuse_train_neigh:
            print(
            ...
            build_train_pool_neighbors(
                ...
                candidate_k=-1 if exhaustive_dtw_cache else cfg.positive_mining_candidate_k,
                pooling=cfg.positive_mining_pooling,
            )
        else:
        ...
        if exhaustive_dtw_cache and train_neigh.get("dtw_mode") != "exact_all_pairs":
            raise RuntimeError(
                f"topk_set_rank requires an exact all-pairs train-pool cache; "
                f"got dtw_mode={train_neigh.get('dtw_mode')!r} at {train_neigh_path}"
            )
        _cached_train_idx = train_neigh.get("train_clip_indices")
        if _cached_train_idx is not None and (
            _cached_train_idx.numel() != len(train_positions)
            or not bool(torch.equal(
                _cached_train_idx.to(torch.int64),
                torch.as_tensor(train_positions, dtype=torch.int64),
            ))
        ):
            raise RuntimeError(
                f"Train-pool DTW neighbor cache at {train_neigh_path} was built "
                "for a different train split than this run's holdout split"
            )
    ...
    # is empty, so this collapses to a single param group.
    ...
    # cached global neighbor IDs/similarities and has no in-batch DTW
    # distribution term.
    ...
    # -- Build the eval-set exact-DTW similarity matrix once.
    ...
    # Checkpoint-selection metric (see `_run_and_log_evaluation`): CKNNA-DTW
    # at `cknna_topk` in trajectory mode; the harmonic mean of DTW R@5 and
    # JEPA R@5 in JEPA mode.
    ...
    # and see each run as a named entry.
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        while True:
            ...
            if not set_rank_mode:
                ...
                # Per-batch exact-DTW similarity matrix (fp32, autocast-disabled).
